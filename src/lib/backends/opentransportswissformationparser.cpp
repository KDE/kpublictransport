...
[[nodiscard]] static float findPlatformSectionStart(const std::vector<PlatformSection> &platform, QStringView sectorName)
{
...
bool OpenTransportSwissFormationParser::parse(const QByteArray &data, const QString &identifierType, int uicCode)
{
    ...
    for (const auto &formationV : formations) {
        std::unordered_map<QString, float> platformLengthMap;
        float maxCoachLength = 0.0f;
        ...
        for (const auto &formationVehicleV : formationVehicles) {
            ...
            const float coachLength = (float)vehicleProperties.value("length"_L1).toDouble();
        ...
        const auto platformLength = std::accumulate(platformLengthMap.begin(), platformLengthMap.end(), 0.0f, [](float l, const auto &sec) {
            return l + sec.second;
        });
        if (platformLength > 0.0f) {
            float platformPos = 0.0f;
            for (auto &sec : platformSections) {
                sec.setBegin(platformPos / platformLength);
                platformPos += platformLengthMap[sec.name()];
                sec.setEnd(platformPos / platformLength);
            }
        ...
        if (vehicle.direction() == Vehicle::Forward) {
            ...
            for (auto &coach : coaches) {
                ...
                coach.setPlatformPositionBegin(pos);
                pos += coach.length() / platformLength;
                coach.setPlatformPositionEnd(pos);
            }
        }
