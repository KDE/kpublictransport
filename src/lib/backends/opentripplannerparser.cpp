...
bool OpenTripPlannerParser::parseLocationFragment(const QJsonObject &obj, Location &loc) const
{
    ...
    if (const auto routes = obj.value("routes"_L1).toArray(); !routes.isEmpty()) {
        ...
        for (const auto &mode : modes) {
            Line line;
            line.setMode(Siri::Mode::fromString(mode.toString(), {}).toMode());
            info.addLine(line);
        }
        loc.setType(Location::Stop);
        loc.setData(info);
    ...
    if (const auto bss = obj.value("rentalVehicleStation"_L1).toObject(); !bss.isEmpty()) {
...
Location OpenTripPlannerParser::parseLocation(const QJsonObject &obj) const
{
    ...
    const auto bikeRental = obj.value("rentalVehicleStation"_L1).toObject();
    // TODO free floating
    ...
    qDebug() << valid << loc.name() << loc.type();
...
OpenTripPlannerParser::RouteData OpenTripPlannerParser::parseLine(const QJsonObject &obj) const
{
    ...
    if (type.isString()) {
        line.setMode(Siri::Mode::fromString(obj.value("transportMode"_L1).toString(), type.toString()).toMode());
