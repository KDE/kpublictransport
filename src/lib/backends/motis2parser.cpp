...
{
    return timestamp ? QDateTime::fromMSecsSinceEpoch(timestamp, QTimeZone::fromSecondsAheadOfUtc(0)) : QDateTime();
}
...
Stopover Motis2Parser::parsePlace(const QJsonObject &obj, bool hasRealTime) const
{
    ...
    Stopover s;
    s.setStopPoint(l);
    s.setScheduledPlatform(obj.value("track"_L1).toString());
    s.setScheduledDepartureTime(parseTime(obj.value("departure"_L1).toInteger()));
    s.setScheduledArrivalTime(parseTime(obj.value("arrival"_L1).toInteger()));
    if (hasRealTime) {
        s.setExpectedDepartureTime(s.scheduledDepartureTime().addMSecs(obj.value("departureDelay"_L1).toInteger()));
        s.setExpectedArrivalTime(s.scheduledDepartureTime().addMSecs(obj.value("arrivalDelay"_L1).toInteger()));
    }
...
std::vector<Journey> Motis2Parser::parseItineraries(const QByteArray &data)
{
    ...
    for (const auto &itineraryV :itineraries) {
        ...
        for (const auto &legV : legs) {
            ...
            s.setScheduledDepartureTime(parseTime(leg.value("startTime"_L1).toInteger()));
            s.setScheduledArrivalTime(parseTime(leg.value("endTime"_L1).toInteger()));
            ...
            if (hasRealTime) {
                ...
                s.setExpectedArrivalTime(s.scheduledDepartureTime().addMSecs(leg.value("arrivalDelay"_L1).toInteger()));
            }
            ...
            // TODO level leg geometry, steps
            if (const auto geometryObj = leg.value("legGeometry"_L1).toObject(); !geometryObj.empty()) {
                QPolygonF poly;
                poly.reserve(geometryObj.value("length"_L1).toInteger());
...
std::vector<Stopover> Motis2Parser::parseStopTimes(const QByteArray &data, bool isArrival)
{
    ...
    for (const auto &stopV : stopTimes) {
        ...
        s.setRoute(route);
        if (isArrival) {
            s.setScheduledArrivalTime(parseTime(stop.value("time"_L1).toInteger()));
            if (stop.value("realTime"_L1).toBool()) {
                s.setExpectedArrivalTime(s.scheduledDepartureTime().addMSecs(stop.value("delay"_L1).toInteger()));
            }
        } else {
            s.setScheduledDepartureTime(parseTime(stop.value("time"_L1).toInteger()));
            if (stop.value("realTime"_L1).toBool()) {
                s.setExpectedDepartureTime(s.scheduledDepartureTime().addMSecs(stop.value("delay"_L1).toInteger()));
            }
        }
