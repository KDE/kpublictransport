...
Stopover Motis2Parser::parsePlace(const QJsonObject &obj, bool hasRealTime) const
{
    ...
    if (hasRealTime) {
        s.setExpectedDepartureTime(s.scheduledDepartureTime().addSecs(obj.value("departureDelay"_L1).toInteger()));
        s.setExpectedArrivalTime(s.scheduledDepartureTime().addSecs(obj.value("arrivalDelay"_L1).toInteger()));
    }
...
std::vector<Stopover> Motis2Parser::parseStopTimes(const QByteArray &data)
{
    ...
    for (const auto &stopV : stopTimes) {
        ...
        line.setName(stop.value("route"_L1).toString());
        ...
        Stopover s;
        s.setRoute(route);
        s.setScheduledDepartureTime(QDateTime::fromMSecsSinceEpoch(stop.value("time"_L1).toInteger())); // TODO arrival when searching the other way around?
        if (stop.value("realTime"_L1).toBool()) {
            s.setScheduledDepartureTime(s.scheduledDepartureTime().addSecs(stop.value("delay"_L1).toInteger()));
        }
