...
Delegates.RoundedItemDelegate {
    ...
    contentItem: Delegates.SubtitleContentItem {
        ...
        subtitle: {
            if (delegate.waitingForLocation) {
                ...
                } else if (!positionSource.ageAcceptable) {
                    return i18nd("kpublictransport", "Waiting for more recent location…");
    ...
    QtPositioning.PositionSource {
        ...
        //readonly property bool gpsSupported: supportedPositioningMethods & QtPositioning.PositionSource.SatellitePositioningMethods
    ...
    QtLocation.GeocodeModel {
        ...
        onLocationsChanged: {
            if (count >= 1) {
                ...
                delegate.location.streetAddress = loc.street + " " + loc.streetNumber;
                delegate.location.postalCode = loc.postalCode
                delegate.location.locality = loc.city;
                delegate.location.addressCountry = I18nLocaleData.Country.fromName(loc.countryCode).alpha2;
                // TODO do we need region?
